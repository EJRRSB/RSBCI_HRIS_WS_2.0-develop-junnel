...
export default function useUsers() {
    ...
    const validateWorkEmail = async(work_email_address) => {
        let response = await axios.post( '/api/v1/validate/work_email_address/', { 'work_email_address': work_email_address }, { "headers": config.headers });
    ...
    const validateUsername = async(uname) => {
        ...
        let response = await axios.post('/api/v1/validate/username/', { 'username': uname }, { "headers": config.headers });
