...
export default function useEmployees() {
    ...
    const getEmployees = async(status) => { 
        let response;
        if (status === 0) {
            response = await axios.post('/api/v1/employee-list');
        } else {
            response = await axios.post('/api/v1/employee-list/' + status);
        }
    ...
    const activateEmployee = async(id) => {
        let response = await axios.post('/api/v1/employee-status/' + id + '/1');
        employee.value = response;
    }

    const declineEmployee = async(id) => {
        let response = await axios.post('/api/v1/employee-status/' + id + '/3');
        employee.value = response;
    }
